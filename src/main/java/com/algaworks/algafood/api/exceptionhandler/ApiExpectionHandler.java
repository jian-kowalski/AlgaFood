...
public class ApiExpectionHandler extends ResponseEntityExceptionHandler {
  ...
  protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex, HttpHeaders headers,
    ...
    String detail = "O corpo da requisicao esta invalido. Verifique a sintaxe para prosseguir";
  ...
  protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers,
    ...
    List<Problem.Field> fields = bindingResult.getFieldErrors().stream().map(fieldError -> Problem.Field.builder()
        .name(fieldError.getField()).userMessage(fieldError.getDefaultMessage()).build()).collect(Collectors.toList());

    Problem problem = createProblemBuilder(status, problemType, detail).userMessage(detail).fields(fields).build();
  ...
  private ResponseEntity<Object> handleInvalidFormat(InvalidFormatException ex, HttpHeaders headers, HttpStatus status,
    ...
    String detail = String.format(
        "A propriedade '%s' recebeu o valor '%s', "
            + "que e de um tipo invalido. Corriga e informe um valor compativel com o tipo %s.",
  ...
  public ResponseEntity<?> hhandleEntidadeNaoEncontrada(EntidadeNaoEncontradaException ex, WebRequest request) {
  ...
  private Problem.ProblemBuilder createProblemBuilder(HttpStatus status, ProblemType problemType, String detail) {
    return Problem.builder().status(status.value()).type(problemType.getUri()).title(problemType.getTitle())
        .userMessage(MSG_ERRO_USUARIO_FINAL).timestamp(LocalDateTime.now()).detail(detail);
  }
}
